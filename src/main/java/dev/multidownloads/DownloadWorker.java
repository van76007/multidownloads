...
/**
 * An implementation of the Callable interface, i.e. a process can be executed independently in a thread and return some result.
 * This class processes 1 download task. The file associated with the task is split into chunks.
 * The chunks are put into queue. A number of threads process the queue in parallel.
...
public class DownloadWorker implements Callable<DownloadTask> {
	 ...
	 * @param progressListener The listener will be updated with download progress, i.e. number of the downloaded bytes / file size
	 */
	...
	public DownloadTask call() throws Exception {
		...
		if (task.getInfor().isSupportMultiPartsDownload()) {
			try {
				...
				logger.warn("No config of NUM_OF_CONCURRENT_CONNECTION. To use the default value {}", NUM_OF_CONCURRENT_CONNECTION);
			}
		}
