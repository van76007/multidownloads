...
/**
 * A class to download a file via FTP protocol if the server supports the REST
 * command The file is split into multiple parts and they can be retrieved
 * concurrently. This class only download 1 part of the file. The class
 * implements the Callable interface so it can be executed in a thread and
 * return result after finish
 * 
 * @see Downloader
 * @author vanvu
 *
...
public class FTPMultiPartsDownloader extends Downloader implements Callable<Segmentation> {
	...
	public FTPMultiPartsDownloader(DownloadInfor infor, Segmentation seg, DownloadListener progressListener, SpeedInformer speedInformer) {
	...
	public Segmentation call() throws Exception {
		...
		try {
			...
			setError(this.seg);
			logger.error("Error in downloading 1 segment of file via FTP. Range: {} - {}. Error: {}", seg.startByte,
					seg.endByte, e.getMessage());
		} finally {
			if (raf != null) {
				try {
					raf.close();
